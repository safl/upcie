 ...
 *
 * Usage:
 *   dmamem_vfio_bar_smoketest <cdev> <region_index> <length>
 *   e.g.
 *   dmamem_vfio_bar_smoketest /dev/vfio/devices/vfio0 1 0x40000000
 *
...
main(int argc, char *argv[])
{
	...
	if (err) {
		fprintf(stderr, "FAIL: iommufd_device_open('%s') err(%d): %s\n", argv[1], err,
			strerror(-err));
		goto out_ioas;
	}
	...
	if (err) {
		fprintf(stderr, "FAIL: iommufd_device_bind() err(%d): %s\n", err, strerror(-err));
		goto out_dev;
	}

	err = iommufd_device_attach(&dev, ioas_id);
	if (err) {
		fprintf(stderr, "FAIL: iommufd_device_attach() err(%d): %s\n", err,
			strerror(-err));
		goto out_dev;
	}
	...
	err = dmamem_from_dmabuf(&dmem, &iommufd, ioas_id, dbuf_fd, length);
	if (err) {
