 ...
 * vfio cdev at /dev/vfio/devices/vfioN. Pass N as argv[1] (or set
 * NVME_DMAMEM_CDEV=/dev/vfio/devices/vfio0 in the environment) so the
 * test does not have to resolve BDF to cdev on its own.
 *
...
main(int argc, char *argv[])
{
	...
	err = dmamem_from_memfd(&dmem, &iommufd, ioas_id, hugepgsz * nhugepages, hugepgsz);
	if (err) {
	...
	err = nvme_controller_open_dmamem(&ctrlr, &ctx, &iommufd, ioas_id, &heap, cdev_path);
	if (err) {
		fprintf(stderr, "FAIL: nvme_controller_open_dmamem() err(%d): %s\n", err,
			strerror(-err));
		goto out_heap;
	}
	...
	nvme_controller_close_dmamem(&ctrlr, &ctx, &heap);
