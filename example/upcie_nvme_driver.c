...
nvme_cleanup(struct nvme *nvme)
{
	...
	if (nvme->backend == NVME_BACKEND_VFIO_IOMMUFD) {
		...
		if (nvme->dm.ctrlr_alive) {
			nvme_controller_close_dmamem_vfio(&nvme->ctrlr, &nvme->dm.ctx, &nvme->dm.heap);
		...
		if (nvme->dm.ioas_alive) {
			iommufd_destroy(&nvme->dm.iommufd, nvme->dm.iommufd.ioas_id);
...
nvme_open_dmamem(struct nvme *nvme, const char *bdf)
{
	...
	err = iommufd_ioas_alloc(&nvme->dm.iommufd);
	if (err) {
	...
	err = dmamem_from_memfd(&nvme->dm.dmem, &nvme->dm.iommufd, heap_size, hugepgsz);
	if (err) {
	...
	err = nvme_controller_open_dmamem_vfio(&nvme->ctrlr, &nvme->dm.ctx, &nvme->dm.iommufd,
					  &nvme->dm.heap, cdev_path);
	if (err) {
		printf("FAILED: nvme_controller_open_dmamem_vfio(); err(%d)\n", err);
		goto fail;
	}
