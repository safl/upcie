...
sequenceDiagram
    ...
    C->>S: ALLOC_IOQPAIR_CQ_AT (depth, registered region, CQ offset)
    S->>S: take SQ and PRP memory from the heap
    S->>K: Create CQ at the client's address, Create SQ
    S-->>C: queue id and the three offsets
    C->>K: I/O; the client mirrors the CQ it placed into the heap copy
...
A client with memory of its own can also ask for a queue whose completion
queue alone lies in a region it registered. The submission queue and the PRP
scratch stay in the server's heap, so the host submits as it always did, and
the controller completes beside the client's data rather than across PCIe into
host memory. The offsets that come back name a completion queue in the heap
the controller never writes; the client keeps it a copy of the one it placed,
so that completions are read the same way whichever queue they land in.
