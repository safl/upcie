...
A server listens on one unix socket per controller it holds, named for the
controller. A client derives the same name and connects. A connection reaches
exactly one controller, which is why no message needs to name one.
...
Attach is the only exchange that carries descriptors. The client receives the
DMA heap and the device, maps both, and reads the runtime record from the heap
at the offset the reply names.
...
sequenceDiagram
    ...
    C->>S: connect to the controller's socket
    C->>S: ATTACH
...
Once attached, a client submits I/O by itself. Everything else is a request,
