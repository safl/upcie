		...
		struct hostmem_heap *heap)
{
	...
	qp->sq = hostmem_dma_element_alloc(qp->heap, 1, nbytes);
	if (!qp->sq) {
		UPCIE_DEBUG("FAILED: hostmem_dma_element_alloc(sq); errno(%d)\n", errno);
		return -errno;
	}
	memset(qp->sq, 0, nbytes);

	qp->cq = hostmem_dma_element_alloc(qp->heap, 1, nbytes);
	if (!qp->cq) {
		UPCIE_DEBUG("FAILED: hostmem_dma_element_alloc(cq); errno(%d)\n", errno);
		hostmem_dma_free(qp->heap, qp->sq);
		return -errno;
	}
