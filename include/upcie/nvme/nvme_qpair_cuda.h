...
nvme_qpair_cuda_enqueue_at_i(struct nvme_qpair_cuda *qp, struct nvme_command *cmd, uint16_t offset)
{
	...
	// Copy word-by-word through a volatile pointer to bypass the per-SM L1
	// data cache. Direct volatile struct assignment is not valid C++.
	volatile uint32_t *src = (volatile uint32_t *)cmd;
	uint32_t *dst = (uint32_t *)&sq[index];
