...
hostmem_heap_block_element_alloc_aligned(struct hostmem_heap *heap, size_t nmemb, size_t size, size_t alignment)
{
	...
	if (size > (size_t)heap->config->hugepgsz || (total_size > (size_t)heap->config->hugepgsz 
		&& (size_t)heap->config->hugepgsz % size != 0)
	) {
		UPCIE_DEBUG("FAILED: Cannot allocate memory; size(%ld) must be aligned to hugepage size(%d)", 
			size, heap->config->hugepgsz);
		errno = ENOMEM;
		return NULL;
	}
	...
	while (block) {
		if (block->free && block->size >= (total_size + alignment)) {
			...
			disalignment = (hpage_remaining - alignment) % size;
...
static inline void *
hostmem_heap_block_element_alloc(struct hostmem_heap *heap, size_t nmemb, size_t size)
{
	return hostmem_heap_block_element_alloc_aligned(heap, nmemb, size, heap->config->pagesize);
}
