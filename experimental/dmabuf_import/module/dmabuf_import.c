 ...
 * device memory from a GPU driver.
 *
 * The ioctls were originally delivered as a patch to the in-tree udmabuf
 * driver, which is where their names and numbers come from. Because Ubuntu
 * ships CONFIG_UDMABUF=y (built into vmlinuz), changing udmabuf's code
 * required rebuilding the kernel. The importer functionality, however, is
 * purely additive and self-contained: it never calls a udmabuf-internal
 * function; it only uses EXPORTED dma-buf core APIs
 * (dma_buf_get/attach/map_attachment_unlocked/... ) plus rbtree. So it is
 * lifted here into its own module that registers its own misc device
 * (/dev/dmabuf_import) and COEXISTS with the stock built-in udmabuf. That
 * makes it buildable + shippable out-of-tree (DKMS) against a stock kernel
 * with no kernel rebuild and no custom kernel.
 *
 * Userspace keeps using the stock /dev/udmabuf for UDMABUF_CREATE, and uses
 * /dev/dmabuf_import for DMABUF_IMPORT_ATTACH / _GET_MAP / _DETACH.
 */
 ...
 * tear down the other's attachment with DMABUF_IMPORT_DETACH. Scoping the tree to the
 ...
 * named at DMABUF_IMPORT_ATTACH time. Without that check a closed-and-reused fd
 ...
 */
static struct dmabuf_import_desc *dmabuf_import_desc_lookup(struct dmabuf_import_ctx *ctx,
							     int dma_buf_fd)
{
	struct dmabuf_import_desc *desc;
	struct dma_buf *dmabuf;

	desc = dmabuf_import_tree_find(&ctx->tree, dma_buf_fd);
	if (!desc)
...
static int dmabuf_import_attach_locked(struct dmabuf_import_ctx *ctx,
			  ...
			  struct device *dev, enum dma_data_direction dir)
{
	...
	tmp = dmabuf_import_desc_lookup(ctx, dma_buf_fd);
	if (!IS_ERR(tmp)) {
	...
	tmp = dmabuf_import_tree_find(&ctx->tree, dma_buf_fd);
	if (tmp) {
	...
	ret = dmabuf_import_tree_insert(&ctx->tree, tmp);
	if (ret)
		goto err;
...
{
	struct dmabuf_import_ctx *ctx = filp->private_data;
	...
	mutex_lock(&ctx->lock);
	ret = dmabuf_import_attach_locked(ctx, &desc, attach.fd, dev, DMA_BIDIRECTIONAL);
	if (!ret)
...
static long dmabuf_import_ioctl_detach(struct file *filp, unsigned long arg)
{
	...
	mutex_lock(&ctx->lock);
	desc = dmabuf_import_tree_find(&ctx->tree, fd);
	if (desc)
...
static long dmabuf_import_ioctl_get_map(struct file *filp, unsigned long arg)
{
	 ...
	 * sg_table belong to the tree, and a concurrent DMABUF_IMPORT_DETACH on
	 * another thread of the same process would otherwise free them under us.
	 */
	mutex_lock(&ctx->lock);
	desc = dmabuf_import_desc_lookup(ctx, get_map.fd);
	if (IS_ERR(desc)) {
		mutex_unlock(&ctx->lock);
		return PTR_ERR(desc);
	}
...
static int dmabuf_import_chrdev_open(struct inode *inode, struct file *filp)
{
	struct dmabuf_import_ctx *ctx;
...
static int dmabuf_import_chrdev_release(struct inode *inode, struct file *filp)
{
	struct dmabuf_import_ctx *ctx = filp->private_data;
	struct dmabuf_import_desc *desc, *tmp;
...
static const struct file_operations dmabuf_import_fops = {
	.owner		= THIS_MODULE,
	.open		= dmabuf_import_chrdev_open,
	.release	= dmabuf_import_chrdev_release,
