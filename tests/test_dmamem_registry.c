 ...
 * ===================================
 *
 * Pure bookkeeping: adoption takes addresses from a caller-supplied table, so
 * nothing here allocates device memory or touches a device. What is checked is
 * that a registered range resolves, an unregistered one does not, and that both
 * survive removal and failure.
 */
...
/**
 * A range enclosing a live backing must be refused too.
 *
 * The lookup can only reuse a backing that encloses the range, so a range
 * enclosing a backing cannot share it. Accepting it would build a second
 * backing over granules the first still owns, which is the state the overlap
 * guard exists to prevent.
 */
static int
test_enclosing_range_is_refused(void)
{
	struct dmamem_registry reg;
	uint64_t lut[GRAN / 4096 * 3];
	const uint64_t first = reg_addr(0);
	...
	assert(!dmamem_registry_adopt(&reg, (void *)(BASE + GRAN), GRAN, lut, 12, NULL));

	if (!dmamem_registry_adopt(&reg, (void *)BASE, GRAN * 3, lut, 12, NULL)) {
		printf("FAILED: a range enclosing a live backing was accepted\n");
		return 1;
	}

	if (reg.lut_phys[(BASE >> GRAN_SHIFT) + 1] != first) {
		printf("FAILED: the refusal disturbed the backing it encloses\n");
		return 1;
	}

	dmamem_registry_term(&reg);
	printf("PASSED: a range enclosing a live backing is refused\n");
	return 0;
}
...
main(void)
{
	...
	if (test_enclosing_range_is_refused()) {
		return 1;
	}
